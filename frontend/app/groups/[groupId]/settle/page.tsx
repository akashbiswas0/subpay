...
export default function SettlePage() {
  ...
  const debtFormatted = currentDebt !== null
    ? token === "PAS"
  ...
  async function handleSubmit() {
    ...
    if (currentDebt !== null && amountPlanck > currentDebt) {
      setValidationError(
        `Amount exceeds debt. Max: ${debtFormatted}`
      );
      return;
    }
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-semibold">Settle Debt</h1>
            ...
            <button
              onClick={() => setAmount(
                token === "PAS"
                  ...
                  : (Number(currentDebt) / Math.pow(10, TOKEN_DECIMALS[token])).toFixed(6)
              )}
